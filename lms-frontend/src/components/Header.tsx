    ...
    <header className="sticky top-0 z-50 border-b border-border bg-card/80 px-4 sm:px-6 lg:px-8 backdrop-blur-lg">
      <nav className="flex items-center justify-between gap-4 h-16 max-w-[1440px] mx-auto">
        ...
        <div className="hidden sm:flex items-center gap-6">
          <Link to="/courses" className="text-sm font-medium text-muted-foreground hover:text-foreground no-underline! transition-colors">
            Courses
          </Link>
          {user && (
            <Link to="/dashboard" className="text-sm font-medium text-muted-foreground hover:text-foreground no-underline! transition-colors">
              Dashboard
            </Link>
          )}
        </div>
