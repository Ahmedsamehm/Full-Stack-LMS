...
export default function ContinueLearningCard({ course, isLoading }: ContinueLearningProps) {
  if (isLoading) {
    return <ContinueLearningSkeleton />
  }

  if (!course) {
    return <EmptyState title="No course in progress" message="Start a course to continue your learning journey." />
  }
    ...
    <Card className="overflow-hidden p-0">
      <div className="flex flex-col md:flex-row">
        <div className="md:w-[40%] h-48 md:h-auto bg-muted relative">
          ...
          <Badge variant="secondary" className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 backdrop-blur-md border border-border/50">
            {course.category}
          </Badge>
        </div>
        ...
        <CardContent className="p-4 sm:p-6 flex-1 flex flex-col justify-center">
          <div className="flex justify-between items-start gap-2 mb-2">
            <h3 className="text-lg sm:text-xl font-semibold text-foreground">{course.title}</h3>
            <DropdownMenu>
          ...
          <p className="text-sm text-muted-foreground mb-4 sm:mb-6 line-clamp-2">{course.description}</p>
          ...
          <div className="mt-auto">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-xs sm:text-sm font-bold text-primary">{course.progress}%</span>
            </div>
