  ...
  return (
    <Card className="p-0 gap-0 overflow-hidden">
      ...
      <CardContent className="p-3 sm:p-4 flex flex-col gap-2">
        ...
        {course.description && <p className="text-xs text-muted-foreground line-clamp-2 mb-1">{course.description}</p>}
  ...
  return (
    <Card className="p-0 gap-0 hover:bg-accent/50 transition-colors cursor-pointer">
      <CardContent className="p-3 sm:p-4 flex items-start gap-3 sm:gap-4">
        <img src={course.thumbnail} alt={course.title} className="size-14 sm:size-16 rounded-lg object-cover shrink-0" />
...
export default function RecommendedCourses({ courses, isLoading }: RecommendedCoursesProps) {
  if (isLoading) {
  ...
  if (courses.length === 0) {
    return <EmptyState title="No recommendations yet" message="Complete more courses to get personalized recommendations." />
  }

  return (
    <div className="flex flex-col gap-3 sm:gap-4">
      {courses.map((course) =>
        course.type === 'large' ? <LargeCard key={course.id} course={course} /> : <SmallCard key={course.id} course={course} />,
      )}
    </div>
  )
}
