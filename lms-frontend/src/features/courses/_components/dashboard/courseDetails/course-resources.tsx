...
const fileIconMap: Record<string, { Icon: React.ComponentType<{ className?: string }>; color: string }> = {
...
export function CourseResources({ resources }: CourseResourcesProps) {
  return (
    <div className="bg-surface-container-lowest rounded-xl shadow-low p-6">
      <h2 className="text-lg font-semibold text-on-surface mb-4">Course Resources</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {resources.map((section) => (
          <div key={section.title}>
            <h3 className="text-xs font-semibold text-secondary mb-3 uppercase tracking-wider">{section.title}</h3>
              ...
              {section.resources.map((resource) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-sm text-on-surface">{resource.name}</span>
                  </li>
                )
              })}
            </ul>
