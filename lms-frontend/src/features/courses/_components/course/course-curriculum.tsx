...
import type { CourseDetailLesson } from '#/schemas'
...
function LessonItem({ lesson }: { lesson: CourseDetailLesson }) {
  ...
  return (
    <div className="border-b border-border last:border-b-0">
      ...
      >
        <ChevronDown
          className={cn('size-4 text-muted-foreground shrink-0 transition-transform duration-200', expanded && 'rotate-0', !expanded && '-rotate-90')}
        />
      ...
      {expanded && (
        <div className="px-5 pb-4 pl-14 space-y-2">
          ...
          {lesson.pdfUrl && (
            <a href={lesson.pdfUrl} className="inline-flex items-center gap-1.5 text-sm text-primary hover:underline">
...
export default function CourseCurriculum({ lessons, totalLessons, totalDuration }: CourseCurriculumProps) {
  return (
    <section>
